...
class App extends React.Component {
    componentDidMount() {
        this.refs.myGrid.on('cellselect', (event) => {
            ...
            let container = document.getElementById('eventLog');
            container.innerHTML = '<div>Selected Cell<br/>Row: ' + event.args.rowindex + ', Column: ' + column.text + ', Value: ' + value + ', Label: ' + displayValue + '</div>';
        });
    ...
    render() {
        ...
        return (
            <div>
                <JqxGrid ref='myGrid'
                    width={600} source={gridAdapter} selectionmode={'singlecell'}
                    autoheight={true} editable={true} columns={columns}
                />
                <div id='eventLog' style={{ fontSize: 13, marginTop: 20, fontFamily: 'Verdana' }} />
            </div>
        )
    }
}
