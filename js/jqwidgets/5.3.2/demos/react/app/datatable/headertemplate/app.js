...
class App extends React.Component {
    ...
    componentDidMount() {
        ...
        let myCheckBox = ReactDOM.render(<JqxCheckBox />, document.getElementById('checkbox'));   

        this.refs.myDataTable.on('rowSelect', (event) => {
            this.setState({ updatingSelectionFromDataTable: true });
            if (!this.state.updatingSelection) {
                myCheckBox.checked(this.allRowsSelected());
            }
            this.setState({ updatingSelectionFromDataTable: false });
        });
        ...
        myCheckBox.on('change', (event) => {
            if (!this.state.updatingSelectionFromDataTable) {
                let args = event.args;
                let rows = this.refs.myDataTable.getRows();
                this.setState({ updatingSelection: true });
                if (args.checked) {
            ...
            [
                {
                    ...
                    renderer: (text, align, height) => {
                        return '<div id="checkbox" style="z-index: 999; margin: 5px; margin-left: 30px; margin-top: 8px; margin-bottom: 3px;"></div>';
                    },
                    // row - row's index.
