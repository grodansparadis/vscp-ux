...
class App extends React.Component {
    componentDidMount() {
        this.refs.myDropDownList.on('change', (event) => {
            let selectedIndex = this.refs.myDropDownList.selectedIndex();
            this.refs.myChart.colorScheme(this.colorSchemes()[selectedIndex].name);
        });
    }
    colorSchemes() {
        let schemes = [
    ...
    render() {
        ...
        let itemTemplate = (index) => {
            if (index == -1) return '';
            let colors = this.colorSchemes()[index].colors;
            let html = '';
        ...
        let selectionTemplate = (element, index) => {
            if (index < 0)
                return;
            let colors = this.colorSchemes()[index].colors;
            let html = '';
        ...
        return (
            <div>
                ...
                <JqxDropDownList ref='myDropDownList'
                    width={180} height={28} selectedIndex={0} itemHeight={35} selectionRenderer={selectionTemplate}
                    source={this.colorSchemes()} displayMember={'name'} renderer={itemTemplate}
                /> 
            </div>
        )
    }
