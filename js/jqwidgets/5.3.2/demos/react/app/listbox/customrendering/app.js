...
class App extends React.Component {
    ...
    updatePanel(index) {
        ...
        let listBoxItemValue = this.refs.myListBox.getItem(index).value;

        let education = '<div style="clear: both; margin: 10px;"><div><b>Education</b></div><div>' + listBoxItemValue + '</div></div>';

        container.innerHTML += education;

        document.getElementById('ContentPanel').innerHTML = container.innerHTML;
    }
    getData() {
        let data = new Array();
    ...
    render() {
        ...
        return (
            <JqxSplitter
                width={600} height={600} panels={[{ size: '40%' }]}
            >
                <div style={{ overflow: 'hidden' }}>
                    <JqxListBox ref='myListBox' style={{ border: 'none' }}
                        width={'100%'} height={'100%'} source={dataAdapter}
                        selectedIndex={0} itemHeight={70} renderer={renderer}
