...
class App extends React.Component {
    componentDidMount() {
        ...
        // Cell Begin Edit
        this.refs.myTreeGrid.on('cellBeginEdit', (event) => {
            let args = event.args;
            // row key
            let rowKey = args.key;
            // row's data.
            ...
            log.innerHTML = 'cellBeginEdit - Row ID: ' + rowKey + ', Column: ' + columnDataField + ', Value: ' + value + '<br/>' + log.innerHTML;
        });
        // Cell End Edit
        this.refs.myTreeGrid.on('cellEndEdit', (event) => {
            let args = event.args;
            // row key
            let rowKey = args.key;
            // row's data.
            ...
            if (columnDataField == "ShippedDate")
                value = dataAdapter.formatDate(value, 'dd/MM/yyyy');
            log.innerHTML = '<br/>cellEndEdit - Row ID: ' + rowKey + ', Column: ' + columnDataField + ', Value: ' + value + '<br/>' + log.innerHTML;
        });
    }
    render () {
        // prepare the data
        ...
        return (
            <div>
                ...
                <div style={{ fontSize: '13px', fontFamily: 'Verdana', width: '800px', marginTop: '20px' }}>
                    <h4>Event Log</h4>
                    <div style={{ maxHeight: '300px', overflow: 'auto' }} id='log'></div>
                </div>
            </div>
        )
    }
