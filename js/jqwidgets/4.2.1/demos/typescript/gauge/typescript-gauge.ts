...
function createGauge(selector, additionalSelector) {
    ...
    let options: jqwidgets.GaugeOptions = {
        ranges: [
    ...
    let myGauge: jqwidgets.jqxGauge = jqwidgets.createInstance(selector, 'jqxGauge', options);

    // initialization of the Input in the Gauge
    let inputOptions: jqwidgets.InputOptions = { disabled: true };
    let myInput: jqwidgets.jqxInput = jqwidgets.createInstance(additionalSelector, 'jqxInput', inputOptions);
