...
// setFilter
//-----------------------------------------------------------------------------
vscpws_Event.prototype.setFilter = function()
{
    var cmd;
                                             ...
                                             vscpdata ) 
{
    ...
    if ( typeof(vscpdata) == 'array' ) {
        for (i=0;i<this.vscpdata.length;i++) {
            cmd += this.vscpdata[i].toString() + ",";
            if (i<vscpdata.length-1) cmd += ",";   // No comma for last
        }
    }
    else ( typeof(vscpdata) == 'string' ) { {
        cmd += vscpdata;
    }
    else {
        Trow "vscpws_Event : event data not a valid type.";
    }
