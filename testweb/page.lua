...
<ul>
  ...
  vscp.write("<li>" .. _VERSION .. " server pages</li>")
  if sqlite3 then
    vscp.write("<li>sqlite3 binding</li>")
  end
  if lfs then
    vscp.write("<li>lua file system</li>")
  end


vscp.write("</ul></p>\r\n")
vscp.write("<p> Today is " .. os.date("%A") .. "</p>\r\n")
vscp.write("<p> URI is " .. vscp.request_info.uri .. "</p>\r\n")

vscp.write("<p>Database example:\r\n<pre>\r\n")
  ...
  -- db:trace(function(data, sql) vscp.write('Executing: ', sql: '\n') end, nil)
  ...
  stmt:bind_values(vscp.request_info.request_method,
                   vscp.request_info.uri,
                   vscp.request_info.remote_port)
  stmt:step()
  stmt:finalize()

  -- Show all previous records
  vscp.write('Previous requests:\n')
  stmt = db:prepare('SELECT * FROM requests ORDER BY id DESC;')
  while stmt:step() == sqlite3.ROW do
    local v = stmt:get_values()
    vscp.write(v[1] .. ' ' .. v[2] .. ' ' .. v[3] .. ' '
          .. v[4] .. ' ' .. v[5] .. '\n')
  end
